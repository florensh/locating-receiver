...
exec('sudo airmon-ng start wlan1', function(error, stdout, stderr) {
  if (!error) {
    ...
    ts = spawn('tshark', ['-i', 'mon0', '-f', 'broadcast && wlan.sa == d0:e1:40:73:89:7c', '-Y', 'wlan.fc.type == 0 && wlan.fc.subtype == 4', '-T', 'fields', '-e', 'frame.time_epoch', '-e', 'wlan.sa', '-e', 'radiotap.dbm_antsignal']);
